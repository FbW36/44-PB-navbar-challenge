  ...
  const getOutput = (arr) => {
    ...
    arr.forEach((item) => {
      if (item["path"]) {
        const newObj = {};
        const subObj = {};
        newObj["params"] = subObj;
        subObj["page"] = item["path"].replace(/[/]/g, "_").slice(1);
        output.push(newObj);
      }
      if (item.options) {
        item.options.forEach((subItem) => {
          if (subItem["path"]) {
            ...
            const subObj = {};
            newObj["params"] = subObj;
            subObj["page"] = subItem["path"].replace(/[/]/g, "_").slice(1);
            output.push(newObj);
          }
          if (subItem.options) {
            subItem.options.filter((thirdLvlItem) => {
              thirdLvlItem["path"];
              const newObj = {};
              const subObj = {};
              newObj["params"] = subObj;
              subObj["page"] = thirdLvlItem["path"].replace(/[/]/g, "_").slice(1);
            output.push(newObj);
            });
          }
        });
